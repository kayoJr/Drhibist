...
if(isset($_POST['submit'])){
    ...
    $amount = $_POST['quant'];
    $today = date("Y-m-d");
    $sql = "SELECT * FROM `system_payment_pharm` WHERE `id` = '$id' AND `date` = '$today'";
    $rs = $conn->query($sql);
    if($rs){
        $row = $rs->fetch_assoc();
        $old_amount = $row['quan'];
        ...
        $sql_upd = "UPDATE `system_payment_pharm` SET `quan` = '$sold', `sub_price`='$new_price' WHERE `id` = '$id' AND `date` = '$today'";
