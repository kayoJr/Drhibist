...
$sel = "SELECT * FROM `cart_test` WHERE `id`='$id' AND `user_id`='$phone_user'";
$pro = $conn->query($sel);
while ($row = $pro->fetch_assoc()) {
    ...
    while ($rows = $medd->fetch_assoc()) {
        ...
        $resl = $conn->query($upd);
        if ($resl) {
            $sql = "DELETE FROM `cart_test` WHERE `id` = '$id' AND `user_id` = '$phone_user'";
            $res = $conn->query($sql);
            if ($res) {
                header("Location: ../Users/Pharmacy/pharmaTest.php?msg=Deleted");
            } else {
                header("Location: ./Users/Pharmacy/pharmaTest.php?err=Failed to Delete");
            }
        }else{
